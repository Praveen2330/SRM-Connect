...
io.on('connection', (socket) => {
  ...
  socket.on('chatMessage', async ({ to, text, roomId }) => {
    ...
    if (!room) {
      ...
      socket.emit('error', { message: 'Invalid room for chat' });
      return;
    }

    if (!room.users.includes(socket.user.id) || !room.users.includes(to)) {
      console.error('Unauthorized chat attempt:', {
        room: room.users,
        from: socket.user.id,
        to
      });
      socket.emit('error', { message: 'Unauthorized chat attempt' });
      return;
    }
    ...
    if (!targetSocket) {
      console.error('Target user not found for chat:', to);
      socket.emit('error', { message: 'Chat recipient disconnected' });
      return;
    }
  ...
  // Handle likes
  socket.on('like', async ({ to, roomId }) => {
    ...
    const room = activeMatches.get(roomId);
    if (!room) {
      console.error('Room not found for like:', roomId);
      socket.emit('error', { message: 'Invalid room for like' });
      return;
    }
    ...
    if (!room.users.includes(socket.user.id) || !room.users.includes(to)) {
      ...
      socket.emit('error', { message: 'Unauthorized like attempt' });
      return;
    }
    ...
    const targetSocket = activeUsers.get(to);
    if (!targetSocket) {
      console.error('Target user not found for like:', to);
      socket.emit('error', { message: 'Like recipient disconnected' });
      return;
    }

    // Forward the like
    targetSocket.emit('like', {
      from: socket.user.id,
      roomId,
      timestamp: new Date().toISOString()
    });
